...
> **Fork additions:** question tools now accept custom `dataExportTag` (human-readable column names), `displayLogic` (question-level show/hide rules), `requestResponse` (soft response prompt, distinct from force response), `validation` overrides, `configuration` (raw Qualtrics `Configuration` object — required for Timing questions, useful for per-type display settings), per-row `rowExportTags` on matrix questions (maps to the question-level `ChoiceDataExportTags` object), and a `choiceDataExportTags` escape hatch on raw `create_question` / `update_question`. `update_question` carries forward `DataExportTag`, `DisplayLogic`, `Configuration`, `ChoiceDataExportTags`, and other fields that the Qualtrics full-replace PUT would otherwise wipe.
